...
- [ ] 官方分享右侧入口采用相近行高、灰色图标文字、蓝色hover及无独立矩形描边；官方分享不被替代。
- [ ] 保留脚本菜单备用入口和SPA重挂载，入口可键盘访问。
- [ ] 识别标准B站及BewlyCat实际浅深模式，打开初始化及打开期间切换都有效；不只依赖可能错过的事件。
- [ ] 识别失效保留上次有效模式、首次未知浅色；不以系统偏好替代页面设置，不新增选择器或持久主题缓存。
- [ ] 面板统一配色且海报保持固定色；记录实际环境版本，验证可精确恢复的真实设置切换，不以CSS模拟宣称通过。
