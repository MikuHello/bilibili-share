...
function renderState(){
 ...
 for(const id of ['copy-text','part','time','detail','markdown'])$('#'+id).disabled=blocked;
...
for(const [id,verb] of [['copy-poster','复制海报'],['copy-text','复制文案'],['download','下载 PNG'],['combined','组合复制']])$('#'+id).addEventListener('click',()=>{$('#status').classList.remove('error');$('#status').textContent=verb==='组合复制'?'已提供图文兼容格式（演示），接收方可能只粘贴其中一种。':verb+'完成（演示）'});
