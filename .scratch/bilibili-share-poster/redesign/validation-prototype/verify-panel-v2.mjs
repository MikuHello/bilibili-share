...
const out=new URL('evidence/panel-v2/',import.meta.url);await mkdir(out,{recursive:true});
const browser=await chromium.launch({headless:true}),page=await browser.newPage({viewport:{width:1440,height:1000}}),errors=[];
page.on('pageerror',e=>errors.push(e.message));
try{
 await page.goto('http://127.0.0.1:8768/?revision=2');await page.waitForLoadState('networkidle');
 assert.equal(await page.locator('details').count(),0);assert.ok(await page.locator('#detail').isVisible());assert.ok(await page.locator('#markdown').isVisible());assert.ok(await page.locator('#combined').isVisible());
 ...
 await page.locator('#detail').check();await page.locator('#markdown').check();assert.match(await page.locator('#text-body').innerText(),/UP 主/);await page.locator('#combined').click();assert.match(await page.locator('#status').innerText(),/接收方可能只粘贴其中一种/);
 ...
 for(const state of ['loading','updating','fallback','copyfailed','error','coverfailed']){
  ...
  assert.equal(await page.locator('#copy-text').isDisabled(),blocked);
 ...
 for(const width of [390,320]){
  await page.setViewportSize({width,height:844});await page.goto('http://127.0.0.1:8768/?revision=2');await page.waitForLoadState('networkidle');assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.locator('.panel').blur();await page.screenshot({path:new URL('mobile-'+width+'-poster.png',out).pathname});
  await page.locator('#combined').scrollIntoViewIfNeeded();const box=await page.locator('#combined').boundingBox();assert.ok(box.x>=0&&box.x+box.width<=width);assert.ok(await page.locator('#detail').isVisible());await page.screenshot({path:new URL('mobile-'+width+'-actions.png',out).pathname});
 }
 assert.deepEqual(errors,[]);await writeFile(new URL('report.json',out),JSON.stringify({passed:true,browser:browser.version(),errors,checks:['visible options','icon download beside poster','two copy actions','6 states','cover-failure text/retry available and image actions disabled','part/time preserved semantics','Escape focus return','390/320 viewport and action access']},null,2));console.log('Panel interaction checks passed.');
