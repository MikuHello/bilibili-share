...
- `npm run check`, `npm test` (66 tests), and `npm run build` pass.
- `scripts/verify-b3-poster.mjs`: 17 PNG cases pass: 12 title/shape combinations, long name/extreme statistics, missing/zero values, long marked target, and two additional real covers. Every PNG was independently decoded with Apple Vision using `verify-poster-qr.swift`.
