...
**Status:** reopened — 已确认B局部收尾，原验收记录保留
...
- [ ] 文案高度自适应，右侧紧凑居中，操作区紧邻文案；取消64px常驻预留，浮层恢复路径及非阻塞行为完整。
- [ ] 详细文案编号紧邻链接且无BV/av前缀，普通/Markdown顺序一致；不改变详细信息默认值。
- [ ] 按新规格补充几何/窄屏/反馈验证并回归，双轴审阅后交付0.3.2。
