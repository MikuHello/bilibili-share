...
async function cloneVideoElement(video, options) {
  ...
  const poster2 = video.poster;
  const contentType = getMimeType(poster2);
  const dataURL = await resourceToDataURL(poster2, contentType, options);
  return createImage(dataURL);
}
...
async function deriveCoverAppearance(src) {
  ...
  const scale = Math.max(280 / cover2.naturalWidth, 360 / cover2.naturalHeight);
  context.drawImage(cover2, (240 - cover2.naturalWidth * scale) / 2, (320 - cover2.naturalHeight * scale) / 2, cover2.naturalWidth * scale, cover2.naturalHeight * scale);
  context.filter = "none";
...
function coverAppearance(snapshot2) {
  let result = appearances.get(snapshot2);
  if (!result) {
    result = deriveCoverAppearance(snapshot2.coverDataUrl);
    appearances.set(snapshot2, result);
    void result.catch(() => appearances.delete(snapshot2));
  }
  return result;
}
...
async function fitContent(poster2, title) {
  ...
  try {
    ...
    await Promise.all(Array.from(poster2.querySelectorAll("img"), (img) => img.decode()));
    ...
    try {
      const editorialStyle = getComputedStyle(poster2.querySelector(".bsp-d-editorial"));
      const available = poster2.clientHeight * 0.26 - parseFloat(editorialStyle.paddingTop) - parseFloat(editorialStyle.paddingBottom);
    ...
    for (const value of poster2.querySelectorAll(".bsp-d-stat-value")) {
...
async function createPoster(model, snapshot2) {
  if (model.coverUnavailable) throw new Error("\u5C01\u9762\u6682\u65F6\u65E0\u6CD5\u52A0\u8F7D");
  const poster2 = element("article", "bsp-poster bsp-default-poster");
  const appearance = await coverAppearance(snapshot2);
  poster2.style.backgroundImage = `url("${appearance.background}")`;
  poster2.style.backgroundSize = "100% 100%";
  poster2.style.color = appearance.ink;
  poster2.setAttribute("aria-label", `${model.title} \u5206\u4EAB\u6D77\u62A5`);
  ...
  const cover2 = image("bsp-d-cover", model.coverDataUrl, "\u539F\u89C6\u9891\u5B8C\u6574\u5C01\u9762");
  ...
  poster2.append(style, mast, cover2, editorial, footer, linkFooter);
  await fitContent(poster2, title);
  return poster2;
}
async function createPosterQr(target2) {
  const data = await import_qrcode.default.toDataURL(target2, { width: 564, margin: 4, errorCorrectionLevel: "M", color: { dark: "#000000", light: "#ffffff" } });
  return image("bsp-d-qr-image", data, `\u4E8C\u7EF4\u7801\uFF1A${target2}`);
}
var fontVersion = 0;
...
async function exportPosterPng(poster2) {
  await document.fonts.ready;
  let cached = exports.get(poster2);
  if (!cached || cached.fontVersion !== fontVersion) {
    const png = toPng(poster2, { width: 1080, height: 1440, pixelRatio: 1, cacheBust: false, style: { transform: "none" } });
    cached = { fontVersion, png };
    exports.set(poster2, cached);
    const entry = cached;
    void png.catch(() => {
      if (exports.get(poster2) === entry) exports.delete(poster2);
    });
  }
  return cached.png;
}
...
function buildSharePoster(snapshot2, shareTarget) {
  const title = requireText(snapshot2.title, "\u89C6\u9891\u6807\u9898");
  const coverDataUrl = snapshot2.coverUnavailable ? "" : requireText(snapshot2.coverDataUrl, "\u89C6\u9891\u5C01\u9762");
  const uploader = requireText(snapshot2.uploader, "UP \u4E3B");
  const bvid = requireText(snapshot2.bvid, "BV \u6807\u8BC6");
  if (!Number.isSafeInteger(snapshot2.aid) || snapshot2.aid <= 0) throw new Error("\u7F3A\u5C11av \u6807\u8BC6");
  const validatedShareTarget = validateShareTarget(shareTarget, bvid);
  return {
    dimensions: { width: 1080, height: 1440 },
    coverDataUrl,
    coverUnavailable: snapshot2.coverUnavailable,
    title,
    uploader,
    bvid,
    aid: snapshot2.aid,
    identity: `${bvid} \xB7 av${snapshot2.aid}`,
    shareTarget: validatedShareTarget,
    stats: [
      { label: "\u64AD\u653E", value: formatCompactStat(snapshot2.stats.views) },
      { label: "\u70B9\u8D5E", value: formatCompactStat(snapshot2.stats.likes) },
      { label: "\u6295\u5E01", value: formatCompactStat(snapshot2.stats.coins) },
      { label: "\u6536\u85CF", value: formatCompactStat(snapshot2.stats.favorites) }
    ]
  };
}

// .scratch/bilibili-share-poster/usage-refinement/polish/prototype/prototype.ts
var import_qrcode2 = __toESM(require_browser(), 1);
var variants = ["A", "B", "C"];
var variant = new URL(location.href).searchParams.get("variant") ?? "B";
if (!variants.includes(variant)) variant = "B";
var descriptions = {
  A: "A \xB7 \u5F53\u524D\u7248\uFF1A\u56FE\u6807\u7B49\u8DDD\u7684\u56DB\u7B49\u5206\u7F51\u683C\uFF0C\u767D\u8272\u4E8C\u7EF4\u7801\u5E95\u5757\uFF1B\u6570\u5B57\u8D8A\u77ED\uFF0C\u540E\u9762\u7684\u7A7A\u767D\u8D8A\u5927\u3002",
  B: "B \xB7 \u63A8\u8350\uFF1A\u900F\u660E\u4E8C\u7EF4\u7801\uFF1B\u626B\u7801\u89C2\u770B\u6536\u8FD1\uFF0C\u5E76\u4E0E\u5DE6\u4FA7\u7EDF\u8BA1\u5171\u7528\u4E00\u6761\u6C34\u5E73\u4FE1\u606F\u884C\u3002\u65E0\u4E2D\u5FC3\u56FE\u6807\u3002",
  C: "C \xB7 \u7A33\u59A5\u5BF9\u7167\uFF1A\u540C\u6837\u6309\u5B8C\u6574\u4FE1\u606F\u7EC4\u7B49\u91CF\u7559\u767D\uFF1B\u4E8C\u7EF4\u7801\u653E\u5728\u53D6\u81EA\u80CC\u666F\u7684\u6D45\u8272\u5E95\u5757\u4E0A\uFF0C\u989C\u8272\u534F\u8C03\u4F46\u4ECD\u770B\u5F97\u51FA\u5E95\u5757\u3002"
};
var sets = { actual: [7969, 170, 34, 313], equal: [8888, 8888, 8888, 8888], mixed: [0, 12e3, 9999, null], extreme: [99999e3, 99999e7, Number.MAX_VALUE, null] };
var blob = await (await fetch("./cover.jpg")).blob();
var cover = await new Promise((resolve) => {
  const r = new FileReader();
  r.onload = () => resolve(String(r.result));
  r.readAsDataURL(blob);
});
var snapshot = { bvid: "BV1QGbD6MEDg", aid: 117102467941527, coverDataUrl: cover, coverUnavailable: false, title: "deepseek harness\u63D2\u4EF6\uFF1Adsh-smooth-stream\u65E0\u7EA7\u4E1D\u6ED1\u6D41\u5F0F\u6E32\u67D3", uploader: "\u55D1\u5520\u7684\u9999\u519C", partNumber: 1, partTitle: null, partIdentified: true, playbackSeconds: 0, wasPlaying: false, stats: { views: 7969, likes: 170, coins: 34, favorites: 313 } };
var target = `https://www.bilibili.com/video/${snapshot.bvid}/`;
var serial = 0;
var poster;
function resize() {
  const width = Math.min(innerWidth - 32, 690);
  const mount = document.querySelector("#mount");
  mount.style.width = `${width}px`;
  mount.style.height = `${width * 4 / 3}px`;
  if (poster) poster.style.transform = `scale(${width / 1080})`;
}
function metrics() {
  ...
  const rect = (n) => {
    ...
    return { left: b.left / scale, right: b.right / scale, width: b.width / scale };
  };
  const icons = stats.map((n) => rect(n.firstElementChild));
  const values = stats.map((n) => rect(n.lastElementChild));
  const gaps = values.slice(0, 3).map((v, i) => +(icons[i + 1].left - v.right).toFixed(2));
  const steps = icons.slice(0, 3).map((v, i) => +(icons[i + 1].left - v.left).toFixed(2));
  return { gaps, iconSteps: steps, iconWidths: icons.map((v) => +v.width.toFixed(2)), fontSizes: stats.map((n) => getComputedStyle(n.lastElementChild).fontSize), overflow: stats.some((n) => n.lastElementChild.scrollWidth > n.lastElementChild.clientWidth + 1) };
}
async function render() {
  const version = ++serial;
  const v = variant;
  const key = document.querySelector("#numbers").value;
  const [views, likes, coins, favorites] = sets[key];
  const s = { ...snapshot, stats: { views, likes, coins, favorites } };
  const next = await createPoster(buildSharePoster(s, target), s);
  if (version !== serial) return;
  poster = next;
  if (v !== "A") {
    const style = document.createElement("style");
    style.textContent = ".bsp-d-stats{display:flex;justify-content:space-between;gap:16.2px}.bsp-d-stat{flex:0 0 auto}.bsp-d-stat-value{overflow:visible;text-overflow:clip}.bsp-d-qr-frame{background:transparent}.bsp-d-information{display:flex;align-items:flex-end;gap:21.6px;width:100%;min-width:0}.bsp-d-qr-frame{padding:12.96px 12.96px 0}.bsp-d-qr-caption{margin-top:0;height:54px;line-height:54px}";
    poster.append(style);
    const footer = poster.querySelector(".bsp-d-footer");
    const group = document.createElement("div");
    group.className = "bsp-d-information";
    group.append(...Array.from(footer.children));
    footer.append(group);
    const qr = poster.querySelector(".bsp-d-qr-image");
    qr.src = await import_qrcode2.default.toDataURL(target, { width: 564, margin: 4, errorCorrectionLevel: "M", color: { dark: "#111820ff", light: "#00000000" } });
    await qr.decode();
    if (v === "C") {
      const bg = new Image();
      bg.src = poster.style.backgroundImage.slice(5, -2);
      await bg.decode();
      ...
      x.drawImage(bg, 0, 0);
      const pixel = x.getImageData(215, 255, 1, 1).data;
      const tint = [...pixel].slice(0, 3).map((n) => Math.round(n * 0.45 + 255 * 0.55));
      poster.querySelector(".bsp-d-qr-frame").style.background = `rgb(${tint.join(",")})`;
    }
  }
  document.querySelector("#mount").replaceChildren(poster);
  resize();
  if (v !== "A") {
    const stats = poster.querySelector(".bsp-d-stats");
    const cells = [...stats.children];
    for (const font of [36.72, 34, 31, 28, 25]) {
      for (const cell of cells) cell.lastElementChild.style.fontSize = `${font}px`;
      const scale = poster.getBoundingClientRect().width / 1080;
      const needed = cells.reduce((sum, c) => sum + c.getBoundingClientRect().width / scale, 0) + 16.2 * 3;
      if (needed <= stats.clientWidth + 0.5) break;
    }
  }
  document.querySelector("#variant").textContent = descriptions[v].split("\uFF1A")[0];
  document.querySelector("#description").textContent = descriptions[v];
  document.querySelector("#metrics").textContent = `\u7EC4\u95F4\u7A7A\u767D\uFF08\u6D77\u62A5\u50CF\u7D20\uFF09\uFF1A${metrics().gaps.join(" / ")}`;
  window.probe = { metrics, export: () => exportPosterPng(poster), poster: () => poster };
}
...
document.querySelector("#previous").addEventListener("click", () => choose(-1));
document.querySelector("#next").addEventListener("click", () => choose(1));
document.querySelector("#numbers").addEventListener("change", () => void render());
