...
const out = '.scratch/bilibili-share-poster/redesign/acceptance/ticket05';
...
try {
  ...
  const combined = page.getByRole('button',{name:'组合复制',exact:true});
  ...
  for (const control of [download,combined,plain,markdown,detail]) await control.waitFor();
  ...
  const layout = await page.evaluate(() => {
    ...
    return {panel:rect('.bsp-panel'),preview:rect('.bsp-preview-frame'),download:byName('下载海报 PNG'),copy:byName('复制海报'),combined:byName('组合复制'),heading:rect('#bsp-dialog-title'),headingSize:getComputedStyle(document.querySelector('#bsp-dialog-title')).fontSize};
  });
  ...
  assert.ok(Math.abs(layout.copy.top-layout.combined.top)<1);
  ...
  assert.equal(await combined.evaluate(n=>document.activeElement===n),true);
  ...
  await page.evaluate(()=>{window.fixture.clipboardFailed=false;window.fixture.combinedFailed=true});
  await combined.click();
  await page.getByRole('status').filter({hasText:'仅复制文案'}).waitFor();
  assert.match(await page.getByRole('status').filter({hasText:'仅复制文案'}).textContent(),/海报.*单独/);
  const expectedPlain = await page.getByLabel('分享文案预览',{exact:true}).textContent();
  assert.equal(await page.evaluate(()=>window.fixture.copiedText.at(-1)),expectedPlain);
  results.push({case:'image-failure-and-combined-plain-fallback',passed:true});

  for (const width of [390,320]) {
    await page.setViewportSize({width,height:740});
    for(const control of [download,plain,markdown,detail,combined]) { await control.scrollIntoViewIfNeeded(); assert.equal(await control.isVisible(),true); }
    await combined.click();
  ...
  assert.equal(await partial.page.getByRole('button',{name:'组合复制',exact:true}).isEnabled(),false);
  ...
  await real.page.getByRole('button',{name:'组合复制',exact:true}).click();
  await real.page.getByRole('status').filter({hasText:'已写入兼容格式'}).waitFor();
  const offered = await real.page.evaluate(async()=> { const item=(await navigator.clipboard.read())[0];return {types:item.types,plain:await(await item.getType('text/plain')).text(),html:await(await item.getType('text/html')).text()}; });
  for(const type of ['image/png','text/plain','text/html']) assert.ok(offered.types.includes(type));
  assert.equal(offered.plain,knownPlain);assert.match(offered.html,/<img[\s>]/);assert.match(offered.html,/<pre[\s>]/);
  ...
  assert.ok((await receiver.textContent()).includes(knownPlain));
  ...
  results.push({case:'real-user-gesture-clipboard-download',png,types:offered.types,downloadName:downloaded.suggestedFilename(),sha256:createHash('sha256').update(bytes).digest('hex'),receiver:'Chromium contenteditable',pasted});
