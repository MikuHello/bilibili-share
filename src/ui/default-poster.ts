...
export async function createDefaultPoster(model: SharePoster, snapshot: GenerationSnapshot): Promise<HTMLElement> {
...
/** Decode offscreen; the caller commits only while its generation is current. */
export async function prepareDefaultPosterTarget(target: string): Promise<(poster: HTMLElement) => void> {
